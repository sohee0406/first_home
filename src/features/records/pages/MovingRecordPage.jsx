...
export default function MovingRecordPage() {
  const navigate = useNavigate();
  const { houses } = useHouse();

  // "입주 전" 체크리스트가 완료된 집
  const movingHouses = houses.filter((house) =>
    isChecklistItemComplete(house, "입주 전"),
  );
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6 pb-24">
      ...
      {/* 목록 */}
      {movingHouses.length > 0 ? (
        <div className="space-y-3">
          {movingHouses.map((house) => {
