...
export default function RegisterStep2({ formData, onNext }) {
  ...
  const [maintenanceFee, setMaintenanceFee] = useState(formData?.maintenanceFee || '')
  const [contractPeriod, setContractPeriod] = useState(formData?.contractPeriod || '')
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button onClick={handleNext} className="bg-green-500 text-white rounded-xl py-4 font-bold mt-2">
        다음
      </button>
    </div>
  )
